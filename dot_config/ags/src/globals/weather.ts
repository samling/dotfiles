...
import { getEnvVar } from 'src/lib/utils.js';
const { EXISTS, IS_REGULAR } = GLib.FileTest;

// TODO: Config file(?) for reading in weather api key and location
const enabled = Variable<boolean>(true);
const key = Variable<string>(getEnvVar('WEATHER_API_KEY'));
const unit = Variable<UnitType>('imperial');
const weatherInterval = Variable<number>(60000);
const location = Variable<string>(getEnvVar('WEATHER_LOCATION'));
